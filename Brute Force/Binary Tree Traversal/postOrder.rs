...
fn main() {
    ...
    let tree_tracer = GraphTracer::new("Traversal Post-order");
    let array_tracer = Array1DTracer::new("Print Post-order");
    ...
    post_order(&tree_tracer, &array_tracer, &logger, &t, 5, None, &index);
    // logger {
...
fn post_order(
    ...
    Tracer::delay();
    // }
    post_order(tree_tracer, array_tracer, logger, t, t[root as usize][0], Some(root), index);

    // logger {
    logger.println(format!(" Going right from {}", root));
    Tracer::delay();
    // }
    post_order(tree_tracer, array_tracer, logger, t, t[root as usize][1], Some(root), index);
