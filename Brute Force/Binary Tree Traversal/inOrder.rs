...
fn main() {
    ...
    let tree_tracer = GraphTracer::new("Traversal In-order");
    let array_tracer = Array1DTracer::new("Print In-order");
    ...
    in_order(&tree_tracer, &array_tracer, &logger, &t, 5, None, &index);
    // logger {
...
fn in_order(
    ...
    Tracer::delay();
    // }
    in_order(tree_tracer, array_tracer, logger, t, t[root as usize][0], Some(root), index);
    ...
    Tracer::delay();
    // }
    in_order(tree_tracer, array_tracer, logger, t, t[root as usize][1], Some(root), index);
}
